...
function deletarnormal($id, $db, $model, $criadopor, $auditoria)
{
...
function password_reset($data, $db, $auditoria)
{
    ...
    $db->query("UPDATE `utilizadores` SET `password` = '$hashedpass' WHERE `id` = '$id'");
...
function getUserToken()
{
    ...
    return \Firebase\JWT\JWT::decode($token, new Firebase\JWT\Key($key, 'HS256'))->data;
}

function existEmail($email, $db)
{
    $query = $db->query("SELECT COUNT(*) AS total FROM users WHERE email = '$email'")->getRow(0)->total;
    if ($query > 0)
        return true;
}
